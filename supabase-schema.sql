...
-- Users / Sellers (extends Supabase auth.users)
...
create table public.products (
  ...
  name text not null,
  slug text not null, -- URL-safe version of name e.g. "nextjs-saas-boilerplate"
  description text,
  price integer not null, -- in paise (₹100 = 10000 paise)
  ...
  created_at timestamptz default now(),
  unique(seller_id, slug) -- no duplicate slugs per seller
);
...
-- Products: public read for published, sellers manage own
create policy "Published products are viewable" on public.products for select using (is_published = true);
...
-- Orders: sellers see their product orders, buyers see nothing (server-side only)
...
-- Atomic increment function for sales_count (avoids race condition on concurrent purchases)
